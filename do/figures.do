...
// Figure. PPE and Past quality

tw ///
   (histogram q2 , fc(gs14) s(0) w(1) lc(none) barw(0.9) discrete yaxis(2)) ///
   (lfitci ppe q2, lw(thick) fc(gray%30) alc(white%0)) ///
   (lpoly ppe q2 if casex == 1 , lw(thin) ) ///
   (lpoly ppe q2 if casex == 9 , lw(thin) ) ///
 , legend(on order(3 "Fitted" 4 "Ordinary" 5 "COVID") r(1) pos(12)) ///
   xtit("Past quality index") xlab(0(1)10) ///
   yscale(alt) ytit("Use of PPE index") ylab(0 1 2 3 4)  yscale(alt ax(2))
///

tw (histogram checklist , s(0) w(.05) barw(.04) lc(none) fc(gs12) frac yaxis(2)) ///
   (lpoly correct checklist if casex == 1 , lw(thick) ) ///
   (lpoly correct checklist if casex == 9 , lw(thick) ) ///
 , legend(on order(2 "Ordinary Case" 3 "COVID Suspicion") pos(12)) ///
   xtit("Checklist index") ///
   yscale(alt) ytit(" ") ylab(0 "0%" .25 "25%" .5 "50%" .75 "75%" 1 "100%") ///
   yscale(alt ax(2)) ytit(" " , ax(2)) ylab(0 "0%" .05 "5%" .1 "10%" .15 "15%" .2 "20%", ax(2))

tw (histogram checklist , s(0) w(.05) barw(.04) lc(none) fc(gs12) frac yaxis(2)) ///
   (lpoly correct checklist if ppe_9 == 0 , lw(thick) ) ///
   (lpoly correct checklist if ppe_9 == 1 , lw(thick) ) ///
   (lpoly correct checklist if ppe_9 == 2 , lw(thick) ) ///
   (lpoly correct checklist if ppe_9 == 3 , lw(thick) ) ///
 , legend(on order(2 "No Mask" 3 "Cloth" 4 "Surgical" 5 "N95") pos(12)) ///
   xtit("Checklist index") ///
   yscale(alt) ytit(" ") ylab(0 "0%" .25 "25%" .5 "50%" .75 "75%" 1 "100%") ///
   yscale(alt ax(2)) ytit(" " , ax(2)) ylab(0 "0%" .05 "5%" .1 "10%" .15 "15%" .2 "20%", ax(2))
